...
public class BoardDAO {
	...
	public void insertBoard(BoardDTO dDTO) throws SQLException {
		DbConn dbCon=DbConn.getInstance("jdbc/dbcp");
		...
		//5.바인드변수 값 설정
			pstmt.setString(1, dDTO.getTitle());
			pstmt.setString(2, dDTO.getContent());
			pstmt.setString(3, dDTO.getIp());
			pstmt.setString(4, dDTO.getId());
		//6.쿼리문 수행 후 결과 얻기
			pstmt.executeUpdate();
			//end if
		} finally {
	...
	public int updateBoard(BoardDTO dDTO) throws SQLException {
		...
		//5.바인드변수 값 설정
			pstmt.setString(1, dDTO.getContent());
			pstmt.setString(2, dDTO.getIp());
			pstmt.setInt(3, dDTO.getNum());
			pstmt.setString(4, dDTO.getId());
		//6.쿼리문 수행 후 결과 얻기
